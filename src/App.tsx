...
export default function App() {
  ...
  // 상품 목록: 이제 기본 상품은 전부 Supabase DB에서 불러오고(아래 useEffect),
  // 여기서는 사용자가 직접 업로드해둔 커스텀 사진(로컬 저장)만 초기값으로 반영함
  const [products, setProducts] = useState<ShoeProduct[]>(() => {
    try {
      ...
      let currentProducts: ShoeProduct[] = [];
  ...
  // (갤러리, 사이즈, 기술배지, 상세스펙 등 확장된 칼럼까지 함께 읽어옴)
  useEffect(() => {
    const fetchDbProducts = async () => {
      ...
      const mapped: ShoeProduct[] = data.map((p: any) => ({
        ...
        colors: (p.colors && p.colors.length > 0) ? p.colors : ['#171717'],
        colorVariants: p.color_variants || undefined,
        sizes: (p.sizes && p.sizes.length > 0) ? p.sizes : [250, 255, 260, 265, 270, 275, 280],
        officialStoreUrl: p.official_store_url || '#',
      }));
  ...
  const handleProductUpdated = (updated: ShoeProduct) => {
